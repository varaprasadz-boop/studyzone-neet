...
- `icons.php` — `icon($name, $extra)` emits `<svg class="icn …"><use href="assets/icons/sprite.svg#$name"/></svg>`. Sprite is `assets/icons/sprite.svg` (Lucide-style line icons). Use this in new UI instead of Unicode emoji. `illus($name, $maxWidth='220px')` inlines an SVG illustration from `assets/illus/` and returns `''` silently if missing — use it inside `<div class="note empty">` blocks for friendly empty states.
