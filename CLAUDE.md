...
- **Generated study content** is stored in `study_content` as JSON keyed by `kind`: `concepts` = `{tagline, groups:[{g, pts:[{t,d,e}]}]}`, `formulas` = `[{t,v}]`, `flashcards` = `[[q,a]]`, `quiz` = `[{q,o,c,ex}]`. Chapters with a `hub_file` open the standalone interactive hub in `assets/hub/`; chapters with `study_content` render in-app via `study_chapter.php`.
